...
from app.schemas.propeties_schema import (
    PropertyDetailsBase,
...
async def create_property(
    property_in: Annotated[PropertyDetailsForm, Depends(PropertyDetailsForm.as_form)],
    images: List[UploadFile] = File(None),
    repo: PropertiesRepository = Depends(get_properties_repo),
    current_user: User = Depends(get_current_user),
    storage_service: S3StorageService = Depends(S3StorageService),
):
    try:
        ...
        if images:
            for img in images:
                url = storage_service.upload_file(
                    img.file, img.filename, img.content_type
                )
...
async def get_my_properties(
    ...
    try:
        properties = await repo.list_properties_by_user(
            current_user.id, skip=skip, limit=limit
        )
        return properties
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Error fetching user properties: {str(e)}"
        )
