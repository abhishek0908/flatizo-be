...
async def request_otp(payload: LoginOTPRequest):
    try:
        ...
        send_otp_email_task.delay(
            recipient_email=payload.email, recipient_name="Flatizoion", otp=otp
        )

        logging.info(f"OTP generated and sent to {payload.email}")
        return LoginOTPResponse(success=True, message="OTP sent successfully")
...
async def verify_otp_route(
    payload: LoginOTPVerifyRequest, user_repo: UserRepository = Depends(get_user_repo)
):
    # 1. Verify OTP
    ...
    token = create_jwt_token({"sub": str(user.user_uuid), "email": user.email})
    return LoginTokenResponse(success=True, message="Login successful", token=token)
